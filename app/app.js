// Dinamik Host Belirleme (Localhost ve Docker ortamında sorunsuz çalışır)
...
// Formdaki verileri JSON formatına dönüştürme
function getFormData() {
    return {
        hotel: document.getElementById("hotel").value,
        market_segment: document.getElementById("market_segment").value,
        lead_time: parseInt(document.getElementById("lead_time").value, 10) || 0,
        arrival_date_week_number: parseInt(document.getElementById("arrival_date_week_number").value, 10) || 1,
        reserved_room_type: document.getElementById("reserved_room_type").value,
        stays_in_weekend_nights: parseInt(document.getElementById("stays_in_weekend_nights").value, 10) || 0,
        stays_in_week_nights: parseInt(document.getElementById("stays_in_week_nights").value, 10) || 0,
        ...
        previous_cancellations: parseInt(document.getElementById("previous_cancellations").value, 10) || 0,
        booking_changes: parseInt(document.getElementById("booking_changes").value, 10) || 0,
        days_in_waiting_list: parseInt(document.getElementById("days_in_waiting_list").value, 10) || 0,
        adr: 100.0
    };
}

// Ana Analiz İşlemi (Tekil Tahmin)
async function executeInference() {
    ...
    try {
        ...
        // Gecikme (Latency) Değeri
        latencyBadge.classList.remove("hidden");
        latencyValue.innerText = `${duration} ms`;

        if (!resCancel.ok || !resPrice.ok) {
            const errCancel = !resCancel.ok ? await resCancel.json() : null;
            const errPrice = !resPrice.ok ? await resPrice.json() : null;
            throw new Error(JSON.stringify(errCancel || errPrice));
        }

        const cancelData = await resCancel.json();
        const priceData = await resPrice.json();

        // 1. İptal Riski Kartı Güncelleme
        ...
        // 2. Dinamik Fiyat Kartı Güncelleme
...
// Buton Olayları
document.getElementById("btnExecute").addEventListener("click", executeInference);
...
document.getElementById("btnReset").addEventListener("click", () => {
    ...
    document.getElementById("riskNote").innerHTML = `<i class="fa-solid fa-circle-info text-slate-500 mt-0.5 text-xs"></i> <span>Parametreleri girip analizi çalıştırınız.</span>`;
});
...
// Toplu CSV İşleme (Batch Scoring)
